...
const [users, setUsers] = useState([]);
    ...
    function getUsers() {
        axios.get('http://localhost:81/KEC-Credit-Automation-DB/displayRequest.php').then(function(response) {
    ...
    return (
        <div className="bg-pELC text-pC h-screen">
            <button className="absolute left-12 top-6" onClick={() => navigate(-1)}> 
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" className="h-4 inline m-2"><path d="M41.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l192 192c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L109.3 256 278.6 86.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-192 192z"/></svg>      
            </button>
            <div className="h-full">
            <div className="flex flex-col items-center text-3xl font-extrabold pt-8 mb-4">
                <h2>REQUESTED EXEMPTIONS</h2>
            </div>
            <table className="table-fixed w-11/12 border-separate border-spacing-1 text-center">
              <thead className="bg-pMC text-pLC border border-pC">
                <tr>
                  <th className="w-1/4 p-0 sm:px-4 py-2 border border-pLC break-words">Roll Number</th>
                  ...
                  <th className="w-1/4 p-0 sm:px-4 py-2 border border-pLC break-words">Academic Course Name</th>
              ...
              {users.map((user, key) =>
                        <tr key={key} className="bg-pLC">
                            <td className="border border-pC">{user.roll}</td>
                            <td className="border border-pC">{user.semester}</td>
                            <td className="border border-pC">{user.type_of_course}</td>
                            <td className="border border-pC">{user.academic_course_code} </td>
                            <td className="border border-pC">{user.academic_course_name} </td>
                            <td className="border border-pC">{user.academic_course_credit}</td>
                            ...
                            <td className="border border-pC" >
                                <button onClick={()=>{}} className="bg-pLC hover:bg-pMC text-pC hover:text-pLC font-bold py-2 px-4 rounded-full whitespace-nowrap	"><p>Approve</p></button>
                            </td>
                        </tr>
                    )}
              </tbody>
